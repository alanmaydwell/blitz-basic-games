...
path(loop,5)=48*Sin(90+loop*2);+midy		; Small Sin
path(loop,6)=48*Cos(90+loop*2);+midy		; Small Cos
path(loop,7)=150*Sin(90+loop);+midy		; Med Sin
path(loop,8)=150*Cos(90+loop);+midy		; Med Cos
path(loop,9)=150*Sin(90+loop/2);+midy	; Big Sin
path(loop,10)=150*Cos(90+loop/2);+midy	; Big Cos
Next 
...
If scroll_y=1 Then scroll_f=(scroll_f+1)Mod(9);Advances animation frame of background image.
...
For loop=0 To 6
z=loop*50
spawn(0,H(5),1,5,z,2,1)
spawn(0,H(5)+60,1,5,z,1,1)
spawn(0,H(5),2,5,z,2,1)
spawn(0,H(5)+60,2,5,z,1,1)
Next 
...
For loop=0 To 3
z=(loop*40)
spawn(0,midy,Rnd(1,2),Rnd(3,10),z,Rnd(1,3),1)
Next 
...
If bem\im=3 Then 
spawn(bem\x+30,bem\y-40,0,0,0,4,2)
spawn(bem\x-30,bem\y-40,0,0,0,4,2)
spawn(bem\x+30,bem\y+40,0,0,0,4,2)
spawn(bem\x-30,bem\y+40,0,0,0,4,2)
...
cc=0
For loop = 1 To 12
spawn(0,midy,1,5,loop*20,4,1)
spawn(0,midy,2,5,loop*20,4,1)
Next
