...
Global im_cd=LoadImage("data\cd3.bmp")
...
Global slowdown=0; Game delay factor (used to slow the game)
...
update_laser
update_exp
plotsqr() ;updates expanding square thing
...
;creates expanding squares
bang(bem\x,bem\y,2,2,0)
bang(bem\x,bem\y,2,2,-8)
bang(bem\x,bem\y,2,2,-16)
...
;Set some square things
;creates expanding squares
For l=0 To -32 Step -2
bang(player_x,player_y,4,4,l)
Next
...
;Expanding square type explosion

;creates expanding square
Function bang(x,y,dx,dy,c)
zob.zob=New zob
zob\count=c
zob\x1=x
zob\y1=y
...
;Updates expanding squares
Function plotsqr() 
For zob.zob = Each zob
Color 0,64,256-zob\count*4
...
If zob\count>64 Then Delete zob
Next
End Function 
...
Text midx,H(15),"Speed reduction: "+slowdown,1,0
Color 255,100,100
Text midx,H(17),"EXIT",1,0
Flip
...
temp=GetKey()
If temp>0 And temp<>13 And temp<>8 And Len(a$)<17 Then 
a$=a$+Chr$(temp)
bang(midx,midy,4,4,0)
bang(midx,midy,4,4,-4)
bang(midx,midy,4,4,-8)
End If 
