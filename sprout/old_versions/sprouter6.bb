...
Global szx=(width/2)-75; x Coordinate of corner "safe zone" for ship to materialise.
Global szy=(height/2)-75; y
...
fnt_big=LoadFont("blaster.ttf",36,False,False,False)
fnt_mid=LoadFont("blaster.ttf",24,False,False,False)
fnt_txt=LoadFont("blaster.ttf",14,False,False,False)
SetFont fnt_txt

;Load Sounds
snd_shoot=LoadSound("shoot.wav")
snd_exp=LoadSound("explosion.wav")
snd_shipexp=LoadSound("crash.wav")
snd_bump=LoadSound("ow.wav")
snd_burp=LoadSound("burp.wav")
snd_food=LoadSound("food_fight.wav")
snd_hooray=LoadSound("allel.wav")
snd_laugh=LoadSound("elaugh.wav")
snd_engine=LoadSound("thrust.wav")
snd_rico=LoadSound("rico.wav")
...
Repeat:x=Rnd(50,width-50):Until (x<szx Or x>(szx+150))
Repeat:y=Rnd(50,height-50):Until (y<szy Or y>(szy+150))
newsprout(x,y,1)
Next
...
sprout\x=x
sprout\y=y
Repeat:sprout\dx#=Rnd(-2,2):Until Abs(sprout\dx#)>0.5
Repeat:sprout\dy#=Rnd(-2,2):Until Abs(sprout\dy#)>0.5
...
Cls
Color c,c,0
Text tx,ty*.5,"AWAITING CLEAR MATERIALISATION ZONE",True,True  
For sprout.blob = Each blob
Rect szx,szy,150,150,0
If sprout\s=1 And ImageRectOverlap(b_sproutoid(sprout\o),sprout\x,sprout\y,szx,szy,150,150) Then safe=False
If sprout\s=0 And ImageRectOverlap(s_sproutoid(sprout\o),sprout\x,sprout\y,szx,szy,150,150) Then safe=False
Gosub updatesprouts
Gosub drawsprouts
Next
Flip
...
Color 255,0,0
Rect width/4,h6-12,315,24 
Color 255,i,i
Text x,h6,"Press space bar to start",True,True
Color 0,255,255
SetFont fnt_txt
Text x,h7,"Hold escape key for a few seconds to quit from game",True,True 
