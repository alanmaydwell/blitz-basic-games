...
If pb\fw=1 DrawImage im_bull1,pb\x,pb\y
...
;collision detection between pod And bullet
	...
	If pb\fw=2 Then pod(i)\status=1;BEM bullets can only change chaser into pod but not visa versa.
...
If bem(i)\tp<>0 Then 
	...
	For pb.bullet = Each bullet
	If pb\fw=1 And ImagesCollide(im_dia,bem(i)\x,bem(i)\y,0,im_bull1,pb\x,pb\y,0)
	...
	For j=0 To npods
	If ImagesCollide(im_dia,bem(i)\x,bem(i)\y,0,im_liz,pod(j)\x,pod(j)\y,0) Then 
