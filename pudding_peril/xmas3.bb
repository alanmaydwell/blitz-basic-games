...
Global im_back=LoadImage("data\backpud.bmp")
Global im_title=LoadImage("data\title.bmp")
...
Global sn_ho=LoadSound("data\ho.wav")
Global sn_hohoho=LoadSound("data\hohoho.wav")
Global sn_shipexp=LoadSound("data\crash3.wav")

;Load Fonts

fnt_txt=LoadFont("data\lumparsky.ttf",20,False,False,False)
SetFont fnt_txt
...
Global maxbeams=6  ;Maximum number of symaltaneous beams allowed
...
set_spids(32,H(3),H(18))     ;Sets up some new stary dots
PlaySound(sn_hohoho)
Else status=1 
End If
...
Function set_baddie(x,y,dx,dy,im)
bem.blob=New blob
If dx=0 Then dx=1
If dy=0 Then dy=1
bem\x=x
...
;Sets up cites.
;Erases first to ensure starting from fresh!
Function set_cities()
;Clears any existing cities
...
;Set up snow
;n number of dots generated at randowm heights between y1 & y2
Function set_spids(n,y1,y2)
For loop=1 To n
dot.spid=New spid
dot\x=Rnd(0,width)
dot\y=Rnd(y1,y2)
...
;Deletes any existing skyline to clear the way
...
;Sort of gravity. 
If count Mod(20)=1 And bem\y>H(2) Then bem\dy=bem\dy+1
...
;Update snow
;Pramters y1 & y2 set upper and lower y coordinates for generating replacement dots.
Function update_spids(y1,y2)
k=player_dx/2
d=1+Abs(k)
...
dot\l=dot\l-1
If dot\l=0 Then Delete dot:set_spids(1,y1,y2)
Next
End Function 
...
If bem\im=1 
For loop=1 To 2
set_baddie(bem\x,bem\y+Rnd(1,30)-15,bem\dx+Rnd(-2,2),bem\dy+Rnd(-1,1),2)
Next 
End If 
If bem\im=2 Then 
For loop=1 To 2
set_baddie(bem\x,bem\y+Rnd(1,20)-10,Rnd(-2,2),-2,3)
Next 
End If 
...
display
Color 0,200,255
Text midx,H(14),"Singer Bonus "+bonus,1,1
...
Color 0,255,100
Text midx,H(7),"New Planet",1,1
Text midx,H(8),"Singers Restored",1,1
Flip
Delay 500
set_skyline
set_cities
...
DrawImage im_ship_s,V(1),H(1)
Text V(2),H(1),lives,1,1

;Plot score
Text V(17),H(1),"Score: "+score
Text V(17),H(2),"Puds: "+nbem

End Function


Function greet()
PlayMusic("Data\sleighrd.Mid")
counter=0
show=1
set_spids(128,0,H(18));Generate snow
pudx=0; x coordinate for giant pudding
Repeat
Cls


Color 120,0,0
update_spids(-10,H(18));Update snow

DrawImage im_back,pudx,pudx/2
pudx=pudx+1 
If pudx>800 Then pudx=-400


Text midx,20,"Oh No! It's another game by Alan Maydwell.",1,1 
DrawImage im_title,midx,H(5)

;Shows either the highscores or the controlls

;Slid-shows various text messages
Select show
Case 1
display_story()
Case 2
...
counter=counter+1
If counter=500 Then counter=0:show=show Mod(4) +1 
...
Text midx,H(10),"CAROL PUD PERIL!",1,1
Color 0,255,255
Text midx,H(11),"The orbiting satellite space bakery has exploded,",1,1
Text midx,H(12),"skattering deadly giant Christmas puddings into the",1,1   
Text midx,H(13),"atmosphere. Can Santa with the aid of Rudolf's ruby",1,1
Text midx,H(14),"laser nose save the imperilled carol singers ?",1,1
End Function 

Function display_info()
Color 255,255,255
Text midx,H(8),"CONTROLS",1,1
Color 0,255,255
Text midx,H(9),"A - up",1,1
Text midx,H(10),"Z - down",1,1
Text midx,H(11),"Right-shift - thrust",1,1
Text midx,H(12),"Return - fire",1,1
Text midx,H(13),"Space - reverse",1,1
Color 0,155,255
Text midx,H(14),"or use a joystick",1,1
Text midx,H(15),"(two firebuttons recommended)",1,1
End Function 
...
Color 255,255,255
Text midx,H(10),"ACKNOWLEDGEMENTS",1,1
Color 0,255,255
Text midx,H(11),"Various anonymous people have unknowingly contributed to this game.",1,1
Text midx,H(12),"Game design inspired by Defender, Asteroids and Missile Command",1,1
Text midx,H(13),"Bitmap graphics downloaded and adapted from internet sites",1,1
Text midx,H(14),"Sound effects and music downloaded from internet sites.",1,1
Text midx,H(15),"Font from a font CD",1,1
End Function 
...
expcol=(expcol+1) Mod(6)             ; Advances the explosion colour index
update_exp
Color 0,50,100:Rect V(6),H(5),250,230,1 

Color 255,0,100
Text midx,H(6),"Congratulations!",1,1
Color 255,200,0
Text midx,H(7),"Your score of "+score,1,1
Color 220,220,0
Text midx,H(8),"has placed you at position "+(np+1),1,1
Color 0,255,100
Text midx,H(9),"in the High Score Table",1,1
col(localcolourthing/100)
Text midx,H(11),"Please type your name:",1,1
...
Color 255,255,255 
Text midx,H(13),"(  "+a$+"  )",1,1 

Flip
If KeyDown(1) End;quit if escape pressed
localcolourthing=localcolourthing Mod(700) +3
